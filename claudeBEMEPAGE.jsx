    ...
    <main className="min-h-screen bg-black text-white overflow-x-hidden">
      ...
      <div className="flex flex-col lg:flex-row min-h-screen pt-20">
        <div className="flex-1 flex flex-col justify-center px-8 md:px-14 lg:px-20 py-16 lg:py-24 max-w-none lg:max-w-[55%]">
          <h1 className="text-[clamp(4rem,12vw,9rem)] font-black italic leading-none tracking-tight mb-6">
            be me
            <span className="inline-block w-3 h-3 md:w-4 md:h-4 rounded-full bg-[#d4f200] ml-1 mb-2 align-middle" />
          </h1>

          <p className="text-white/40 text-sm md:text-base font-mono leading-relaxed mb-14 max-w-sm">
          ...
          <div className="space-y-2 max-w-xl">
            {timeline.map((item, i) => (
              <div key={i} className="flex items-start gap-2 group">
                <span className="text-white/20 mt-0.5 shrink-0 text-xs">›</span>
                <p className="text-white/70 text-sm leading-relaxed group-hover:text-white transition-colors duration-200">
                  {item.year && (
                    <span className="text-white font-semibold mr-1">{item.year},</span>
                  )}
                  {item.text}
                </p>
              </div>
        ...
        <div className="lg:flex-1 flex items-center justify-center lg:justify-end relative overflow-hidden">
          <div className="relative w-full h-[50vh] lg:h-screen max-w-lg lg:max-w-none">
            <Image
              src="/banner.png"
              alt="Gaurav"
              fill
