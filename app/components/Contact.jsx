...
const socialLinks = [
  ...
  {
    label: "LinkedIn",
    value: links.linkedin.replace("https://", "").replace("www.", "").replace(/\/$/, ""),
    href: links.linkedin,
    icon: (
  ...
  {
    label: "GitHub",
    value: links.github.replace("https://", "").replace("www.", ""),
    href: links.github,
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
          ...
          <div className="space-y-3">
            {socialLinks.map((link) => (
              <a
                key={link.label}
                href={link.href}
                className="flex items-center gap-4 card group hover:border-muted transition-colors duration-200"
              >
                <span className="text-text-secondary group-hover:text-accent transition-colors">
                  {link.icon}
                </span>
                <div className="flex-1 min-w-0">
                  <p className="font-mono text-xs text-text-tertiary">{link.label}</p>
                  <p className="text-text-primary text-sm truncate group-hover:text-accent transition-colors">
                    {link.value}
                  </p>
                </div>
                <svg className="w-4 h-4 text-text-tertiary group-hover:text-accent transition-colors flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
